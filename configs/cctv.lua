-- Fixed cameras the MDT can look through: banks, 24/7s, liquor stores, hardware shops
-- and gun stores. Watching one moves the VIEWING officer's own camera to that spot, the
-- way a CCTV monitor does, and puts it back when they leave. Nobody else is affected.
-- MDT 可以调看的固定摄像头：银行、24/7 便利店、酒水店、五金店和枪店。观看某个
-- 摄像头时，观看警员自己的视角会移动到该位置（就像监控显示器一样），退出时恢复。
-- 其他玩家不受任何影响。
--
-- The coordinates came from this server's own shop and bank data, so a camera sits where
-- the store actually is rather than where a generic list guessed. Re-run the generator if
-- you move a shop. `coords` is where the camera hangs, `look` is what it points at.
-- 坐标来自本服务器自己的商店和银行数据，因此摄像头位于店铺实际所在位置，而不是
-- 通用列表猜测的位置。移动店铺后请重新运行生成器。`coords` 是摄像头悬挂位置，
-- `look` 是它对准的方向。
--
-- 注意：`label` 是纯显示文本（已汉化为中文）；`category` 既是分组键又是前端配色
-- 键（web/src/apps/mdt/CctvPane.tsx 的 ART 映射），必须保持英文原值，改中文会导致
-- 分组标题变中文的同时缩略图全部回退为默认灰色。
return {
    -- Whether the CCTV section works at all.
    -- 固定监控板块是否启用（总开关）。
    Enabled = true,

    -- Seconds a camera view is held before it drops the streaming focus, so a viewer
    -- flicking between cameras does not thrash the world stream.
    -- 视角在丢掉串流焦点前保持的秒数，避免观看者在摄像头之间快速切换时把世界
    -- 流传输来回折腾。
    FocusGraceSeconds = 3,

    -- Field of view at rest. Lower is more zoomed in.
    -- 静止时的视野（FOV）。数值越低放得越大。
    --
    -- Wide on purpose. A security camera is not a telephoto lens: it is bolted to a ceiling to see
    -- a whole room at once, and anything under about 90 here frames a doorway rather than a shop.
    -- Operators zoom IN from this when they want a face.
    -- 刻意用广角。监控摄像头不是长焦镜头：它装在天花板上是为了一次看到整个房间，
    -- 这里低于 90 左右的话画面只能框住一个门口而不是整家店。操作员想看脸时会在此
    -- 基础上放大。
    Fov = 100.0,

    -- How far an operator may swing a camera off its resting aim, and how far it zooms. These are
    -- the defaults; any camera below may override PanDegrees, TiltUp, TiltDown, ZoomMinFov and
    -- ZoomMaxFov on its own entry, which is how you mix dome cameras with cameras bolted to a wall.
    -- 操作员可以把摄像头从默认朝向转动多少、能放多大。以下为默认值；下面任何一个
    -- 摄像头都可以在自己的条目里覆盖 PanDegrees、TiltUp、TiltDown、ZoomMinFov 和
    -- ZoomMaxFov，这样就能混用球机和固定在墙上的枪机。
    --
    -- PanDegrees is measured EACH WAY from the resting aim, so 70 is a 140 degree arc. Set it to
    -- 360 (or anything at or above it) for a dome camera that spins freely with no stops, and to 0
    -- to pin a camera dead ahead so it can only tilt and zoom.
    -- PanDegrees 从默认朝向向每一侧计算，所以 70 表示 140 度的弧形范围。设为 360
    -- （或更大）表示无死角自由旋转的球机；设为 0 则把摄像头固定在正前方，只能
    -- 俯仰和变焦。
    Controls = {
        PanDegrees  = 360.0,  -- free spin by default; lower it to bolt a camera down 默认自由旋转；调低可固定摄像头
        TiltUp      = 22.0,   -- above centre 中心以上
        TiltDown    = 34.0,   -- below centre 中心以下
        ZoomMinFov  = 30.0,   -- most zoomed in 最大放大（视野最窄）
        ZoomMaxFov  = 110.0,  -- most zoomed out, wide enough to hold a whole room 最大缩小（广角，足以容纳整个房间）
        LookSpeed   = 1.35,   -- degrees per frame at full stick/mouse deflection 摇杆/鼠标推满时每帧转动的度数
        ZoomSpeed   = 2.2,    -- fov change per scroll step 滚轮每格的视野变化量
    },

    Cameras = {
        -- Bank 银行
        -- Any camera may override the Controls above. This one is a dome on the ceiling, so it
        -- spins; a camera in a doorway might take PanDegrees = 45 instead.
        -- 任何摄像头都可以覆盖上面的 Controls。这是装在天花板上的球机，所以可以
        -- 旋转；门口的摄像头可以改用 PanDegrees = 45。
        { id = 'pacific_standard_vinewood', label = '太平洋标准银行，好麦坞', category = 'Bank', PanDegrees = 360.0,
          coords = vec3(241.97, 230.35, 108.64), look = vec3(241.44, 227.19, 106.89) },
        { id = 'fleeca_legion_square', label = '弗莱卡银行，军团广场', category = 'Bank',
          coords = vec3(312.66, -283.55, 56.51), look = vec3(313.84, -280.58, 54.76) },
        { id = 'fleeca_alta_st', label = '弗莱卡银行，阿尔塔街', category = 'Bank',
          coords = vec3(148.13, -1045, 31.72), look = vec3(149.46, -1042.09, 29.97) },
        { id = 'fleeca_burton', label = '弗莱卡银行，波顿', category = 'Bank',
          coords = vec3(-352.23, -54.32, 51.39), look = vec3(-351.23, -51.28, 49.64) },
        { id = 'fleeca_del_perro', label = '弗莱卡银行，德尔佩罗', category = 'Bank',
          coords = vec3(-1210.8, -334.91, 40.13), look = vec3(-1211.9, -331.9, 38.38) },
        { id = 'fleeca_banham_canyon', label = '弗莱卡银行，班汉峡谷', category = 'Bank',
          coords = vec3(-2957.96, 482.75, 18.05), look = vec3(-2961.14, 483.09, 16.3) },
        { id = 'fleeca_harmony_route_68', label = '弗莱卡银行，哈莫尼 / 68号公路', category = 'Bank',
          coords = vec3(1174.88, 2711.4, 40.44), look = vec3(1174.8, 2708.2, 38.69) },
        { id = 'blaine_county_savings_paleto_bay', label = '布雷恩郡储蓄银行，佩立托湾', category = 'Bank',
          coords = vec3(-109.93, 6473.24, 33.98), look = vec3(-112.22, 6471.01, 32.23) },

        -- 24/7 便利店
        { id = '24_7_1', label = '24/7 便利店 1号店', category = '24/7',
          coords = vec3(28, -1345, 31.84), look = vec3(25.7, -1347.3, 30.09) },
        { id = '24_7_2', label = '24/7 便利店 2号店', category = '24/7',
          coords = vec3(-3036.41, 588.2, 10.25), look = vec3(-3038.71, 585.9, 8.5) },
        { id = '24_7_3', label = '24/7 便利店 3号店', category = '24/7',
          coords = vec3(-3239.17, 1003.44, 15.18), look = vec3(-3241.47, 1001.14, 13.43) },
        { id = '24_7_4', label = '24/7 便利店 4号店', category = '24/7',
          coords = vec3(1730.96, 6416.46, 37.38), look = vec3(1728.66, 6414.16, 35.63) },
        { id = '24_7_5', label = '24/7 便利店 5号店', category = '24/7',
          coords = vec3(1700.29, 4926.7, 44.41), look = vec3(1697.99, 4924.4, 42.66) },
        { id = '24_7_6', label = '24/7 便利店 6号店', category = '24/7',
          coords = vec3(1963.78, 3742.26, 34.69), look = vec3(1961.48, 3739.96, 32.94) },
        { id = '24_7_7', label = '24/7 便利店 7号店', category = '24/7',
          coords = vec3(550.09, 2674.09, 44.5), look = vec3(547.79, 2671.79, 42.75) },
        { id = '24_7_8', label = '24/7 便利店 8号店', category = '24/7',
          coords = vec3(2681.55, 3282.42, 57.59), look = vec3(2679.25, 3280.12, 55.84) },
        { id = '24_7_9', label = '24/7 便利店 9号店', category = '24/7',
          coords = vec3(2560.24, 384.35, 110.97), look = vec3(2557.94, 382.05, 109.22) },
        { id = '24_7_10', label = '24/7 便利店 10号店', category = '24/7',
          coords = vec3(375.85, 327.86, 105.91), look = vec3(373.55, 325.56, 104.16) },

        -- Liquor Store 酒水店
        { id = 'liquor_store_1', label = '酒水店 1号店', category = 'Liquor Store',
          coords = vec3(1138.11, -979.98, 48.77), look = vec3(1135.81, -982.28, 47.02) },
        { id = 'liquor_store_2', label = '酒水店 2号店', category = 'Liquor Store',
          coords = vec3(-1220.62, -904.68, 14.68), look = vec3(-1222.91, -906.98, 12.93) },
        { id = 'liquor_store_3', label = '酒水店 3号店', category = 'Liquor Store',
          coords = vec3(-1485.25, -376.81, 42.51), look = vec3(-1487.55, -379.11, 40.76) },
        { id = 'liquor_store_4', label = '酒水店 4号店', category = 'Liquor Store',
          coords = vec3(-2965.94, 393.21, 17.39), look = vec3(-2968.24, 390.91, 15.64) },
        { id = 'liquor_store_5', label = '酒水店 5号店', category = 'Liquor Store',
          coords = vec3(1168.32, 2711.23, 40.51), look = vec3(1166.02, 2708.93, 38.76) },
        { id = 'liquor_store_6', label = '酒水店 6号店', category = 'Liquor Store',
          coords = vec3(1394.86, 3606.98, 37.33), look = vec3(1392.56, 3604.68, 35.58) },
        { id = 'liquor_store_7', label = '酒水店 7号店', category = 'Liquor Store',
          coords = vec3(-1391.11, -604.32, 32.67), look = vec3(-1393.41, -606.62, 30.92) },

        -- YouTool 五金店（品牌名保留英文）
        { id = 'youtool_1', label = 'YouTool 五金店 1号店', category = 'YouTool',
          coords = vec3(2750.3, 3475.3, 58.02), look = vec3(2748, 3473, 56.27) },
        { id = 'youtool_2', label = 'YouTool 五金店 2号店', category = 'YouTool',
          coords = vec3(345.29, -1295.96, 34.86), look = vec3(342.99, -1298.26, 33.11) },

        -- Ammunation 武装国度（Ammu-Nation）
        { id = 'ammunation_1', label = '武装国度 1号店', category = 'Ammunation',
          coords = vec3(-659.88, -932.66, 24.18), look = vec3(-662.18, -934.96, 22.43) },
        { id = 'ammunation_2', label = '武装国度 2号店', category = 'Ammunation',
          coords = vec3(812.55, -2155.3, 31.97), look = vec3(810.25, -2157.6, 30.22) },
        { id = 'ammunation_3', label = '武装国度 3号店', category = 'Ammunation',
          coords = vec3(1695.74, 3762.46, 37.06), look = vec3(1693.44, 3760.16, 35.31) },
        { id = 'ammunation_4', label = '武装国度 4号店', category = 'Ammunation',
          coords = vec3(-327.94, 6086.18, 33.8), look = vec3(-330.24, 6083.88, 32.05) },
        { id = 'ammunation_5', label = '武装国度 5号店', category = 'Ammunation',
          coords = vec3(254.93, -47.7, 72.29), look = vec3(252.63, -50, 70.54) },
        { id = 'ammunation_6', label = '武装国度 6号店', category = 'Ammunation',
          coords = vec3(24.86, -1107.59, 32.15), look = vec3(22.56, -1109.89, 30.4) },
        { id = 'ammunation_7', label = '武装国度 7号店', category = 'Ammunation',
          coords = vec3(2569.99, 296.68, 111.08), look = vec3(2567.69, 294.38, 109.33) },
        { id = 'ammunation_8', label = '武装国度 8号店', category = 'Ammunation',
          coords = vec3(-1115.28, 2700.91, 20.9), look = vec3(-1117.58, 2698.61, 19.15) },
        { id = 'ammunation_9', label = '武装国度 9号店', category = 'Ammunation',
          coords = vec3(844.74, -1031.12, 30.54), look = vec3(842.44, -1033.42, 28.79) },

    },
}
